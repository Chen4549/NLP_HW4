...
def create_augmented_dataloader(args, dataset):
    ...
    # 1) Original train (tokenize exactly like main)
    original_train = dataset["train"].map(tokenize_function, batched=True)
    original_train = original_train.remove_columns(["text"])
    original_train = original_train.rename_column("label", "labels")
    original_train.set_format("torch")

    # 2) 5k transformed train examples (sample -> transform -> tokenize)
    ...
    # 3) Concatenate original + transformed
